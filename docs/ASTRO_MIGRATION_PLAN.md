...
2026-04-28に `RFP.md` を現行Astro版の正本として更新した。新RFPには `books.ts` / `projects.ts` / `BaseLayout.astro` / `BookCard` / `ProjectCard` などの素直な構成案が含まれるが、これは将来の整理候補であり、現在の実装を即座に置き換える指示ではない。

現在の本線は、`src/pages/index.astro` が1ページの骨格を持ち、`src/data/` がセクション別データ、`src/scripts/` が演出・描画、`src/pages/*.js.ts` がJS/CSSの互換配信を担う構成である。RFPと実装が食い違う場合は、現実の構成と公開安定性を優先し、必要な差分だけを小さく取り込む。
...
## Phase E: RFP alignment

目的: 更新されたRFPの情報設計を、現行の1ページ構造と衝突しない形で反映する。

- Libraryを「販売棚」ではなく「問いの結晶」として扱う文脈を保つ。
- Projectsを「制作物一覧」ではなく「問いから生まれた実験」として見せる。
- Research Logを本格ブログ化する前に、トップページ内の小さなログとして育てる。
- Visionは完成済みサービスではなく、未完成の構想として正直に見せる。
- `books.ts` / `projects.ts` のような名前に寄せる場合も、既存の `content.js` 互換配信と描画契約を壊さない。
- CSS / JS / Astroの整理は、RFP反映作業の一部として扱う。見た目の追加だけが改善ではない。
- 1時間サイクルでは、同じ目的に閉じるなら、データ境界、描画、スタイル、検証保護をまとめて動かしてよい。半端な中間状態を残すより、確認できる一塊まで進めることを優先する。
