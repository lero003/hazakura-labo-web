...
- Next: preview 可能な環境で、Projects から Vision、Research Log へ続けてスクロールし、現在地点灯とモバイル6分割ナビの視線移動が自然につながるか確認する。
...
- Focus: Access / Structure
- Changed: 内部リンクのスムーススクロールで、hash更新と到着演出だけで終わらず、到着先セクションへ `tabindex="-1"` を補って `preventScroll` 付きでフォーカスを渡すようにした。phase gate には、巡回路クリック後にフォーカスが対象へ移る契約を追加した。
- Learned: 見た目の参道が整っていても、フォーカスが出発地点に残ると、キーボードや読み上げでは庭を進んだ感覚が途切れる。導線を増やさず、スクロール後の文脈だけを渡すと、1ページの巡回が少し完成品に近づく。
- Next: GUI権限のある環境で、Tab操作から「記録」「構想」「はじまりへ戻る」を辿り、到着演出とフォーカス移動のタイミングが急すぎないか確認する。

## 2026-04-29

- Focus: Structure
- Changed: 終端フッターのロゴ、帰還導線、終端巡回路、Kindle入口を `src/components/FooterSection.astro` へ切り出し、`index.astro` はページ章立てと起動scriptを読むシェルに寄せた。Kindle入口は書籍データの先頭アクションから参照し、phase gate でフッターがコンポーネント化・データ参照されたまま保たれる契約を追加した。
- Learned: 終端は見た目の余韻だけでなく、Heroへ戻る小径、巡回路、外部読書入口が集まる保守上の結び目でもある。ここを部品へ閉じると、次に帰還演出や終端コピーを磨く時にページ全体を掘らずに済む。
- Next: `index.astro` に残る常設ナビも、フッターと対になる巡回路コンポーネントへ分けるほど軽くなるか、見た目を変えずに1回だけ点検する。

## 2026-04-29

- Focus: Structure
- Changed: ナビとフッターに重複していたCSS葉桜マークの6要素を `src/components/LogoMark.astro` へ切り出し、`index.astro` では同じ部品を2箇所で呼ぶだけにした。phase gate も、ロゴマークがコンポーネント経由で2箇所に描画される契約へ更新した。
- Learned: 小さな意匠ほど直書きで複製されると、次に花弁や葉の演出を足す時に入口と終端がずれやすい。ブランド印を一つの部品に閉じると、見た目を変えずに遊びの差し込み口だけを細くできる。
- Next: ナビとフッター以外にも同じ意味の装飾小物が重複していないか、`index.astro` に残る終端周りの部品境界を1つだけ点検する。

## 2026-04-29

- Focus: Visual / Structure
- Changed: ナビとフッターの始点マークをOS絵文字の桜から、CSSで描く小さな葉桜マークへ置き換えた。花弁、葉、芯を同じ `.logo-mark` 部品で管理し、夜テーマの色もCSS変数で差し替える。phase gate には絵文字ロゴへ戻らない保護を追加した。
- Learned: 巡回路や橋が整ってくるほど、最初と最後に見えるブランド印のOS依存な揺れが完成度の弱さとして目立つ。情報を増やさず、印だけを部品化すると、庭の入口が少し私物感のある意匠に寄る。
- Next: GUI権限のある環境で、デスクトップと390px前後のモバイル幅を実表示し、CSS葉桜マークがナビ文字より強く出すぎないか確認する。
...
- Next: preview可能な環境で、Quote 前カードをモバイル幅で見て、4札の遅延が詩へ入る前の余韻として強すぎないか確認する。
...
- Next: Vision 入口のホバー/フォーカスによる一時照明と、ボタンによる固定照明の視覚差が十分かを実機幅で見る。
...
- Next: Vision 入口のカード全体クリックを残すべきか、ボタン操作へさらに寄せるべきかを実機幅で確認する。

## 2026-04-30

- Focus: Motion / Access
- Changed: Projects 終端の `構想の入口へ` から Vision に着地した時、受付ガイド全体だけでなく四つの入口種別が順番に灯る到着演出を追加し、reduced-motion では無効化した。phase gateにもこの着地契約を追加した。
- Learned: 導線の終点をセクション全体ではなく次に触る小さな操作面へ合わせると、情報を増やさずに「どこへ渡されたか」が読みやすくなる。
- Next: preview可能な環境で Projects 終端から Vision 入口までを実機幅で確認し、到着演出が強すぎないかを見る。
...
- Focus: Access / Motion / Structure
- Changed: Projects入口灯を押した時に、棚を絞るだけでなく代表カードへスクロールし、カードへフォーカスを渡すようにした。phase gateにも共有スクロールターゲット経由の到着契約を追加した。
- Learned: 入口灯は上部で状態を変えるだけだと「灯した」感が弱い。代表カードまで視線とフォーカスを運ぶと、情報を増やさずに導線の実感が出る。
- Next: preview可能な環境で、入口灯クリック後のスクロール位置がモバイル幅でカード上部を隠しすぎないか確認する。

## 2026-05-01

- Focus: Access / Structure
- Changed: Projects入口灯から代表カードへ移動する時の到着オフセットを `getEntryTargetOffset` に分け、狭い幅ではカード上に余白を多めに残すようにした。phase gateにも入口灯のレスポンシブ到着契約を追加した。
- Learned: 入口灯の導線は、棚フィルター同期だけでなく「着いた瞬間にカード見出しが読める」ことまで含めて保護すると、モバイルの参道が詰まりにくい。
- Next: preview可能な環境で 390px 幅の入口灯を押し、カード上端の余白が広すぎないかだけ目視する。

## 2026-05-01

- Focus: Structure
- Changed: Projects入口灯と棚ガイドのモバイル横スクロールCSSを共有セレクタへ寄せ、スクロールバー、snap、レール位置の重複を削った。
- Learned: 入口灯と棚ガイドは役割が違っても、狭い幅での「横に続く小さな参道」は同じ責務として扱える。共有化しておくと、次に入口灯の強弱を調整しても棚ガイド側の affordance を壊しにくい。
- Next: preview可能な環境で Projects入口灯と棚ガイドを 390px / desktop 幅で横断し、スクロールバーとレールの見え方が強すぎないか確認する。

## 2026-05-01

- Focus: Structure / Access
- Changed: Projects の棚ボタンだけを押した時は入口灯を `aria-pressed` にしないよう、入口灯の選択状態を代表カードの `target` がある時だけ点灯する契約へ分けた。phase gateにもこの役割分離を追加した。
- Learned: 入口灯は導線、棚ボタンは表示状態という役割を分けた後は、同じ lane でも押下状態を共有しすぎない方が、Projects 冒頭の強弱が読みやすい。
- Next: preview可能な環境で入口灯から代表カードへ移動し、到着後に棚ボタンだけを押し直した時の見た目の残り香が強すぎないか見る。

## 2026-05-01

- Focus: Access / Motion / Structure
- Changed: Projects 入口灯から代表カードへ着地した後、狭い幅やタッチ端末では 2.4 秒で入口灯の点灯とカードの `入口灯` 印だけを自動解除するようにした。棚フィルター状態は維持し、phase gateで解除契約を保護した。
- Learned: 入口灯は目的地を知らせる短い合図として効かせ、棚の絞り込みはそのまま残す方が、モバイルでカード本文へ戻りやすい。
- Next: preview可能な環境で 390px 幅の入口灯を押し、照明が解けた後も棚ボタンの選択状態だけで現在地が伝わるか見る。

## 2026-05-02

- Focus: Structure / Access
- Changed: Projects入口灯と棚ガイドがモバイル幅で共有する横レールの列幅・余白・隙間を `--project-mobile-rail-*` へ寄せ、タッチ端末で横にも縦にも自然にパンできる契約をCSSに明示した。phase gateにもこの共有レール契約を追加した。
- Learned: Projects入口灯は導線、棚ガイドは状態確認だが、狭い幅では同じ「横に続く参道」として読まれる。共有トークンを置くと、次回の強弱調整で片方だけ意図せず崩すリスクを下げられる。
- Next: preview可能な環境で 390px 幅のProjects入口灯と棚ガイドを横断し、スクロールバーとレール線の主張が強すぎないか見る。

## 2026-05-02

- Focus: Structure / Access
- Changed: Projects入口灯とVision入口ガイドで別々に持っていた「狭い幅・タッチ端末では到着照明を短く解く」判定を `HazakuraInteractionEnvironment` へ寄せ、入口灯のスクロール余白も同じ環境ヘルパーから読むようにした。配信順とphase gateにも共有契約を追加した。
- Learned: セクションごとの照明演出は見た目が違っても、モバイルでは同じ「合図は短く、本文へ戻す」ルールで動く。環境判定を1箇所に置くと、次の演出追加で解除時間や幅条件が散らばりにくい。
- Next: preview可能な環境で Projects入口灯とVision入口の自動解除を続けて触り、解除後の現在地表示が薄すぎないか確認する。

## 2026-05-02

- Focus: Access / Structure
- Changed: Projects入口灯と棚ガイドの共有モバイル横レールに端のフェードを足し、横に続く参道の手がかりをスクロールバーだけに頼らない形へ調整した。480px以下で縦積みになる入口灯はフェードを解除し、phase gateで共有フェード契約を保護した。
- Learned: モバイルの横レールは表示要素を増やさなくても、端の処理だけで「まだ続く」感と庭の奥行きを出せる。入口灯が縦積みへ切り替わる幅では、同じ装飾を外す方が読みやすい。
- Next: preview可能な環境で 390px 幅のProjects入口灯と棚ガイドを見て、端フェードがテキストを削りすぎないか確認する。
